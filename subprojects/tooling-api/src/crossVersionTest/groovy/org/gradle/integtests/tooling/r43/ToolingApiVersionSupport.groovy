...
"""
    ...
    // build-receipt.properties was introduced in Gradle 1.1
    def minBuildReceiptVersion = GradleVersion.version('1.1')
    def minConsumerPropertyVersion = GradleVersion.version('1.2')
    ...
    def currentProviderMessage(String version) {
        return currentVersionMessage(GradleVersion.version(version), minBuildReceiptVersion)
    }

    def currentConsumerMessaget(String version) {
        return currentVersionMessage(GradleVersion.version(version), minConsumerPropertyVersion)
    }

    String clientUnsupportedMessage(String version) {
        return "Support for tooling API client older than 2.0 has been removed. ${currentConsumerMessaget(version)}You should upgrade your tooling API client to version 2.0 or later."
    }
    ...
    String clientDeprecationMessage(String version) {
        return "Support for tooling API client older than 3.0 is deprecated and will be removed in 5.0. You are currently using ${version}. You should upgrade your tooling API client."
    }
    ...
    def buildViaScript() {
        packageExecutable('.newBuild().setStandardError(System.err).setStandardOutput(System.out).forTasks("noop").run()')
